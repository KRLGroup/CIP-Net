...
from networks.pipnet_utils.pipnet import PIPNet #Aggiunto
from networks.pipnet_utils.util.args import get_optimizer_nn # aggiunto
from networks.pipnet_utils.util.data import get_dataloaders # aggiunto
...
def main(argv=None):
    ...
    parser.add_argument('--disable_pretrained', action='store_true', help='FOR PIPNET: When set, the backbone network is initialized with random weights instead of being pretrained on another dataset).')
    parser.add_argument('--pipnetdataset', default='CUB-200-2011', help='FOR PIPNET: dataset for pretraining')
    ...
    # Args -- Network
    from networks.network import LLL_Net, LLL_Net_PPNet, LLL_Net_PIPNet
    use_pipnet = True if 'pipnet' in args.network else False
    args.network = 'resnet34'
    print("USE PIPNET:",use_pipnet)
    pipnet_loss = None
    if use_pipnet:
        ...
        if args.nc_first_task is not None:
            ...
            num_cls = args.num_classes*args.num_tasks #IN PIPNET LE DETERMINA IN ALTRO MODO
            
        init_model = construct_PIPNet(args=args, num_classes=num_cls)
        # IN PIPNET FA ANCHE:
        init_model = init_model.to(device=device)
        init_model = nn.DataParallel(init_model, device_ids = gpus)
        print("INIT MODEL DATAPARALLEL MODULES")
        print(init_model.module)
        
    elif 'protopnet' in args.network:
    ...
    Appr_ExemplarsDataset = Appr.exemplars_dataset_class() #PENSAVO CHE ICICLE FOSSE EXEMPLAR-FREE??
    ...
    trn_loader, val_loader, tst_loader, psh_loader, taskcla = get_loaders(args.datasets, args.num_tasks,
                                                                          ...
                                                                          repeat_task_0=args.repeat_task_0,
                                                                          use_pipnet=use_pipnet) ######## aggiunto
    ...
    #######################PIPNET##############################
    scheduler_net = None
    pipnet_loss, pipnet_pretrain_dict = None, {}
    epochs_pretrain = 1
    freeze_epochs = 10
    if use_pipnet:
        trainloader, trainloader_pretraining, trainloader_normal, trainloader_normal_augment, \
        projectloader, testloader, test_projectloader, classes = get_dataloaders(args, device) 
        optimizer_net, optimizer_classifier, params_to_freeze, params_to_train, params_backbone = get_optimizer_nn(init_model, args)  
        pipnet_loss = nn.NLLLoss(reduction='mean').to(device)
        scheduler_net = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_net,
                        T_max=len(trainloader_pretraining)*epochs_pretrain,
                        eta_min=0.0005/100., last_epoch=-1)
        
        pipnet_pretrain_dict = {"epochs_pretrain":epochs_pretrain,
    ...
    utils.seed_everything(seed=args.seed)
    if use_pipnet:
        net = init_model #ICICLE+pipnet 
    elif isinstance(init_model, PPNet) or isinstance(init_model, TesNet):
        net = LLL_Net_PPNet(init_model, remove_existing_head=not args.keep_existing_head) #ICICLE 
        # PRATICAMENTE è tipo così: NET = LLL_Net_PPNet(LLL_Net(TesNet))
    else:
    ...
    utils.seed_everything(seed=args.seed)
    appr = Appr(model=net, device=device, use_pipnet=use_pipnet, pipnet_loss=pipnet_loss, **appr_kwargs)

    # GridSearch
    print("PREPARA GRIDSEARCH?")
    if args.gridsearch_tasks > 0:     
        print("ENTRATO DENTRO PRIMO GS")
    ...
    for t, (_, ncla) in enumerate(taskcla):
        ...
        # GridSearch
        print("INIZIA GRIDSEARCH")
        if t < args.gridsearch_tasks:
            print("ENTRATO DENTRO SECONDO GS")
        ...
        # Train
        if use_pipnet:
            scheduler_classifier, optimizer_net, optimizer_classifier, params_to_freeze, params_to_train, params_backbone = None, None, None, None, None, None
            if t == 0:
                appr.pretrain_pipnet(**pipnet_pretrain_dict)
            # SECOND TRAINING PHASE
            # re-initialize optimizers and schedulers for second training phase
            optimizer_net, optimizer_classifier, params_to_freeze, params_to_train, params_backbone = get_optimizer_nn(init_model, args)            
            scheduler_net = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_net,T_max=len(trainloader)*args.nepochs,eta_min=0.0005/100.)
            ...
            for param in init_model.module.parameters():
                param.requires_grad = False
            for param in init_model.module._classification.parameters():
                param.requires_grad = True

            #############################àSTA ROBA VA MESSA NEL LOOP DI ICICLE PER IL TRAINING####################
            #######################################################################################################
            ########################################################################
            
        appr.train(t, trn_loader[t], val_loader[t], psh_loader[t], use_pipnet, scheduler_classifier, **pipnet_pretrain_dict)
        print('-' * 108)
##########EVAL LO FACCIO NEL TRAIN LOOP (INCREMENTAL LEARNING.PY)

#         # Test
#         for u in range(t + 1):
#             print("TEST SECTION in main_incremental.py")
#             test_loss, acc_taw[t, u], acc_tag[t, u], ppnet_losses_tu = appr.eval(u, tst_loader[u], device=device)
...
#             if args.ppnet_eval:
#                 eval_loss = appr.ppnet_eval(u, tst_loader[u])
