...
class Appr(Inc_Learning_Appr_PPNet):
    ...
    def train_loop(self, t, trn_loader, val_loader, push_loader=None, pipnet=False, scheduler_classifier=None, epochs_pretrain=1, 
                   ...
                   optimizer_net=None, optimizer_classifier=None, scheduler_net=None, device='cpu'):
        ...
        super().train_loop(t, trn_loader, val_loader, push_loader, pipnet=pipnet, scheduler_classifier=scheduler_classifier, 
                           epochs_pretrain=epochs_pretrain, freeze_epochs=freeze_epochs,
                           trainloader_pretraining=trainloader_pretraining, params_to_freeze=params_to_freeze, 
                           params_to_train=params_to_train, params_backbone=params_backbone,
    ...
    def train_epoch(self, t, trn_loader, e, at_push=False): #NON VIENE USATO?
        """Runs a single epoch"""
        print("inizio train epoch in icicle")
        self.model.train()
        ...
        for images, targets in trn_loader: #images è una tupla ora
            xs1 = None
            xs2 = None
            outputs = None
            if self.use_pipnet:
                print("ENTRATO NEL FOR CON PIPNET")
                xs1 = images[0]
                xs2 = images[1]
                
                outputs = self.model(torch.cat([xs1, xs2]))
                proto_features, pooled, out = outputs
                ...
                # OUTPUTS [torch.Size([80, 500, 7, 7]), torch.Size([80, 50]), torch.Size([80, 500]), torch.Size([500, 50])]
            ...
            print("USING CRITERION IN TRAIN EPOCH (ICICLE.PY)")
    ...
    def eval(self, t, val_loader):
        ...
        with torch.no_grad():
            ...
            for images, targets in val_loader:
                ...
                logits = [outputs[i][1] for i in range(len(outputs))]
                min_distances = [outputs[i][2] for i in range(len(outputs))]
                print("USING CRITERION IN EVAL (ICICLE.PY)")
                entropy_loss = self.criterion(t, logits, targets.to(self.device), distances, distances_old)
                # Log
