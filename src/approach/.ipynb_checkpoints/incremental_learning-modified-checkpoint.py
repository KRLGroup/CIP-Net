...
from networks.pipnet_utils.test import eval_pipnet
        ...
        if self.warmup_epochs and t > 0:
            ...
            for e in range(self.warmup_epochs):
                ...
                with torch.no_grad():
                    ...
                    self.model.eval()
                    for images, targets in trn_loader: #SEMBREREBBE CHE FACCIANO UNA EVAL PER OGNI EPOCA
                        outputs = self.model(images.to(self.device))
                        loss = self.warmup_loss(outputs[t], targets.to(self.device) - self.model.task_offset[t])
...
class Inc_Learning_Appr_PPNet(Inc_Learning_Appr):
    ...
    def __init__(self, model, device, use_pipnet = False, pipnet_loss = None, nepochs=100, lr=0.05, lr_min=1e-4, lr_factor=3, lr_patience=5,
        ...
        self.use_pipnet = use_pipnet
        self.pipnet_loss = pipnet_loss
...
#     def get_optimizer_pipnet(self, args: argparse.Namespace) -> torch.optim.Optimizer:
...
#         #     raise ValueError("this optimizer type is not implemented")
    ...
    def pretrain_pipnet(self, epochs_pretrain, freeze_epochs, trainloader_pretraining, params_to_freeze,
                        params_to_train, params_backbone, optimizer_net, optimizer_classifier, scheduler_net, device):
        lrs_pretrain_net = []
        ...
        for epoch in range(1, epochs_pretrain+1):
            ...
            ###################### VEDI STA PARTE###################################################
            # Pretrain prototypes #VEDI STA 
            train_info = self.train_pipnet(trainloader_pretraining, optimizer_net, optimizer_classifier, scheduler_net, None,
                                           epoch, device, pretrain=True, finetune=False)
            lrs_pretrain_net+=train_info['lrs_net']
            plt.clf()
            plt.plot(lrs_pretrain_net)
            plt.savefig(os.path.join("./src/networks/pipnet_utils/",'lr_pretrain_net.png'))
        ...
        #                os.path.join(os.path.join("./src/networks/pipnet_utils/", 'checkpoints'), 'net_pretrained'))
        #     net.train()
        # with torch.no_grad():
        #     if 'convnext' in args.network and epochs_pretrain > 0:
        #         topks = visualize_topk(net, projectloader, len(classes), device, 'visualised_pretrained_prototypes_topk', args)
      
    def train(self, t, trn_loader, val_loader, push_loader=None, use_pipnet=False, scheduler_classifier=None, 
              epochs_pretrain=10, freeze_epochs=10, trainloader_pretraining=None, params_to_freeze=None, params_to_train=None,
              params_backbone=None, optimizer_net=None, optimizer_classifier=None, scheduler_net=None, device='cpu'):
        """Main train structure"""
        self.train_loop(t, trn_loader, val_loader, push_loader, pipnet=use_pipnet, scheduler_classifier=scheduler_classifier, 
                        epochs_pretrain=epochs_pretrain, freeze_epochs=freeze_epochs, trainloader_pretraining=trainloader_pretraining,
                        params_to_freeze=params_to_freeze, params_to_train=params_to_train, params_backbone=params_backbone, 
                        optimizer_net=optimizer_net, optimizer_classifier=optimizer_classifier, scheduler_net=scheduler_net,
                        device=device)
        if not self.use_pipnet:
            self.post_train_process(t, trn_loader)
                
    def train_pipnet(self, train_loader, optimizer_net, optimizer_classifier, scheduler_net, 
                     scheduler_classifier, epoch, device, pretrain=False,
                     finetune=False, progress_prefix: str = 'Train Epoch'):
        ...
        if pretrain:
            align_pf_weight = (epoch/self.nepochs)*1.
        ...
        for i, (x) in train_iter:    
            ...
            # print("OUTPUTS DIMS")
            # print(proto_features.shape, pooled.shape, out.shape)
            loss, acc = self.calculate_loss_pipnet(proto_features, pooled, out, ys, align_pf_weight, t_weight, unif_weight, cl_weight, 
                                              self.model.module._classification.normalization_multiplier, pretrain, finetune,
            ...
            if not pretrain:
                ...
                scheduler_classifier.step(epoch + (i/iters))
    ...
    def calculate_loss_pipnet(self, proto_features, pooled, out, ys1, align_pf_weight, t_weight, unif_weight,
                       cl_weight, net_normalization_multiplier, pretrain, finetune, train_iter, print=True, EPS=1e-10):
        ...
        a_loss_pf = (align_loss(embv1, embv2.detach())+ align_loss(embv2, embv1.detach()))/2. #ALIGNMENT LOSS L_A nel paper
        ...
        if not pretrain:
            ...
            class_loss = self.pipnet_loss(F.log_softmax((softmax_inputs),dim=1),ys) #L_C IN PAPER
    ...
    def train_loop(self, t, trn_loader, val_loader, push_loader=None, pipnet=False, scheduler_classifier=None, epochs_pretrain=1,
                   freeze_epochs=1, trainloader_pretraining=None, params_to_freeze=None, params_to_train=None, params_backbone=None,
                   optimizer_net=None, optimizer_classifier=None, scheduler_net=None, device='cpu'): #STA USANDO QUESTO
        """Contains the epochs loop"""
        print("TASK N.",t)
        lr = self.lr
        best_loss = np.inf
        ...
        self.nepochs = self.nepochs+1 if self.use_pipnet else self.nepochs
        for e in range(self.nepochs):
            # Train
            clock0 = time.time()
            print("HEY FRATM", "train_loop function in incremental_learning.py")
            if self.use_pipnet:
                epochs_to_finetune = 3 #during finetuning, only train classification layer and freeze rest.
                ...
                if (e==self.nepochs or e%30==0) and self.nepochs>1:
                    # SET SMALL WEIGHTS TO ZERO
                    with torch.no_grad():
                        # torch.set_printoptions(profile="full")
                        self.model.module._classification.weight.copy_(torch.clamp(self.model.module._classification.weight.data - 0.001, min=0.)) 
                        print("Classifier weights: ", self.model.module._classification.weight[self.model.module._classification.weight.nonzero(as_tuple=True)],
                              (self.model.module._classification.weight[self.model.module._classification.weight.nonzero(as_tuple=True)]).shape, flush=True)
                        # if args.bias:
                        #     print("Classifier bias: ", self.model.module._classification.bias, flush=True)
                        torch.set_printoptions(profile="default")
                        
                train_info = self.train_pipnet(trn_loader, optimizer_net, optimizer_classifier, scheduler_net,
                                          scheduler_classifier, e, device, pretrain=False, finetune=finetune)
                lrs_net+=train_info['lrs_net']
                lrs_classifier+=train_info['lrs_class']
                
                # Evaluate model
                eval_info = eval_pipnet(self.model, val_loader, e, device) #, log)
                # log.log_values('log_epoch_overview', e, eval_info['top1_accuracy'], eval_info['top5_accuracy'], 
                ...
                with torch.no_grad():
                    # self.model.eval()
                    # torch.save({'model_state_dict': self.model.state_dict(), 'optimizer_net_state_dict': optimizer_net.state_dict(), 
                    ...
                    #     self.model.eval()
                        # torch.save({'model_state_dict': net.state_dict(), 'optimizer_net_state_dict': optimizer_net.state_dict(), 
                        #             'optimizer_classifier_state_dict': optimizer_classifier.state_dict()}, 
                        #            os.path.join(os.path.join(args.log_dir, 'checkpoints'), 'net_trained_%s'%str(e)))            

                    # save learning rate in figure
                    plt.clf()
                    plt.plot(lrs_net)
                    plt.savefig(os.path.join("./src/networks/pipnet_utils/",'lr_net.png'))
                    plt.clf()
                    plt.plot(lrs_classifier)
                    plt.savefig(os.path.join("./src/networks/pipnet_utils/",'lr_class.png'))
            ...
            if self.use_pipnet:
                if eval_info['test_accuracy'] > best_acc:
                    ...
                    patience = self.lr_patience
                    print("UPDATE BEST ACC IN PIPNET")
                else:
                    patience -= 1
                    if patience <= 0:
                        # if it runs out of patience, reduce the learning rate
                        ...
                        # reset patience and recover best model so far to continue training
                        patience = self.lr_patience
                        for i in range(len(self.optimizer_net.param_groups)): #NON SO QUALE OPTIMIZER USARE PER PIPNET (optimizer_net o
                                                                              # optimizer_classifier)
                            self.optimizer_net.param_groups[i]['lr'] /= self.lr_factor
                        # self.model.set_state_dict(best_model)
    ...
    def train_epoch_pipnet(self, t, trn_loader, e): #NON LA USO PER ORA
        """Runs a single epoch"""
        print("inizio train epoch")
        ...
        with torch.no_grad():
            if args.state_dict_dir_net != '':
                ...
                self.model.module._multiplier.requires_grad = False
                try:
                    optimizer_net.load_state_dict(checkpoint['optimizer_net_state_dict']) 
                except:
                    pass
                if torch.mean(self.model.module._classification.weight).item() > 1.0 and \
                    torch.mean(self.model.module._classification.weight).item() <3.0 and \
                    torch.count_nonzero(torch.relu(self.model.module._classification.weight-1e-5)).float().item() >            0.8*(num_prototypes*len(classes)):
                #assume that the linear classification layer is not yet trained (e.g. when loading a pretrained backbone only)
                    ...
                    torch.nn.init.normal_(self.model.module._classification.weight, mean=1.0,std=0.1) 
                    torch.nn.init.constant_(self.model.module._multiplier, val=2.)
                    print("Classification layer initialized with mean", 
                          torch.mean(self.model.module._classification.weight).item(), flush=True)
                #     if args.bias: #in pipnet è false
                #         torch.nn.init.constant_(self.model.module._classification.bias, val=0.)
                ...
                self.model.module._add_on.apply(init_weights_xavier)
                torch.nn.init.normal_(self.model.module._classification.weight, mean=1.0,std=0.1) 
                # if args.bias: #in pipnet è false
                #     torch.nn.init.constant_(self.model.module._classification.bias, val=0.)
                torch.nn.init.constant_(self.model.module._multiplier, val=2.)
                ...
                print("Classification layer initialized with mean", torch.mean(self.model.module._classification.weight).item(), flush=True)
    ...
    def train_epoch(self, t, trn_loader, e, at_push=False): #NON USA QUESTO MA QUELLO DENTRO ICICLE
        """Runs a single epoch"""
        self.model.train()
        if self.fix_bn and t > 0:
            self.model.freeze_bn()
