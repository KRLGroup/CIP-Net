...
class BaseDatasetCIPNet(BaseDataset):
    """Characterizes a dataset for PyTorch -- this dataset pre-loads all paths in memory"""

    def __init__(self, data, transform, class_indices=None, name='cvc',
                 transform1=None, transform2=None, transform_noaug=None, test=False):
        """Initialization"""
        super(BaseDatasetCIPNet, self).__init__(data, transform, class_indices=None, name='cvc')
        self.test = test
    ...
    def __getitem__(self, index):
        ...
        if not self.test:
            x = self.transform1(x)
            return (self.transform2(x), self.transform2(x)), y #MODIFICATO AGGIUNTO
        else:     
            return self.transform_noaug(x), y
        
# class BaseDatasetCIPNet2(BaseDataset):
#     """Characterizes a dataset for PyTorch -- this dataset pre-loads all paths in memory"""

#     def __init__(self, data, transform, class_indices=None, name='cvc',
#                  transform1=None, transform2=None, transform_noaug=None, test=False):
#         """Initialization"""
#         super(BaseDatasetCIPNet2, self).__init__(data, transform, class_indices=None, name='cvc')
#         self.test = test
#         if transform1 != None and transform2 != None:
#             self.transform1 = transform1
#             self.transform2 = transform2
...
def get_data(path, num_tasks, nc_first_task, validation, shuffle_classes, class_order=None):
    ...
    data = {}
    taskcla = []
    # print("STO IN GET DATA",flush=True)
    ...
    # Debugging
    # print(f"Loaded {len(trn_lines)} training samples from train.txt",flush=True)
    # print(f"Loaded {len(tst_lines)} testing samples from test.txt",flush=True)
    # print(f"Loaded {len(psh_lines)} push samples from push.txt",flush=True)

    # Print a few lines to ensure the file contents are correct
    # print(f"First 5 training samples: {trn_lines[:5]}",flush=True)
    # print(f"First 5 testing samples: {tst_lines[:5]}",flush=True)
    # print(f"First 5 push samples: {psh_lines[:5]}",flush=True)
    # sys.stdout.flush()
    ...
    print(f"Classes per task: {cpertask}",flush=True)
    ...
    for this_image, this_label in trn_lines:
        ...
        # print("THIS TASK", this_task,flush=True)
        data[this_task]['trn']['x'].append(this_image)
        data[this_task]['trn']['y'].append(this_label - init_class[this_task])
    # print("PRIMA DATA TRAIN",flush=True)
    # print(f"Training: {len(data[0]['trn']['x'])} samples",flush=True)

    # ALL OR TEST
    for this_image, this_label in tst_lines:
        # print("THIS LABEL_1", this_label, flush=True)
        if not os.path.isabs(this_image):
            this_image = os.path.join(path, this_image)
        this_label = int(this_label)
        if this_label not in class_order:
        ...
        # If shuffling is false, it won't change the class number
        this_label = class_order.index(this_label)
        # print("THIS LABEL_2", this_label, flush=True)
        ...
        # print("THIS LABEL_3", this_label - init_class[this_task], flush=True)
...
def get_data2(path, num_tasks, nc_first_task, validation, shuffle_classes, class_order=None):
    """Prepare data: dataset splits, task partition, class order"""
    print("GET DATA2")
    data = {}
    idxs = {j:{"trn":{},"tst":{}} for j in range(num_tasks)}
    taskcla = []
    # print("STO IN GET DATA",flush=True)
    # read filenames and labels
    if "Cars" in path:
        ...
        with open(os.path.join(path, 'train.txt')) as f:
            for line in f:
                ...
                if not line:
                    continue
                *path_parts, label = line.split()
                paths.append(" ".join(path_parts))
                labels.append(label)
        trn_lines = np.array([list(pair) for pair in zip(paths, labels)])  # if the rest of your code expects pairs
        paths, labels = [], []
        with open(os.path.join(path, 'test.txt')) as f:
            for line in f:
                ...
                *path_parts, label = line.split()
                paths.append(" ".join(path_parts))
                labels.append(label)
        tst_lines = np.array([list(pair) for pair in zip(paths, labels)])  # if the rest of your code expects pairs
        # print(tst_lines, flush =True)
        paths, labels = [], []
        with open(os.path.join(path, 'push.txt')) as f:
            for line in f:
                line = line.strip()
                if not line:
                    continue
                *path_parts, label = line.split()
                paths.append(" ".join(path_parts))
                labels.append(label)
        psh_lines = np.array([list(pair) for pair in zip(paths, labels)])  # if the rest of your code expects pairs
    ...
    for this_image, this_label in trn_lines:
        if not os.path.isabs(this_image):
            this_image = os.path.join(path, this_image)
        this_label0 = int(this_label)
        if this_label0 not in class_order:
            continue
        # If shuffling is false, it won't change the class number
        this_label = class_order.index(this_label0)
        ...
        data[this_task]['trn']['x'].append(this_image)
        data[this_task]['trn']['y'].append(this_label - init_class[this_task])
        idxs[this_task]['trn'][this_label - init_class[this_task]] = this_label0
    # print("PRIMA DATA TRAIN",flush=True)
    # print(f"Training: {len(data[0]['trn']['x'])} samples",flush=True)
    ...
    for this_image, this_label in tst_lines:
        ...
        data[this_task]['tst']['x'].append(this_image)
        data[this_task]['tst']['y'].append(this_label - init_class[this_task])
        idxs[this_task]['tst'][this_label - init_class[this_task]] = this_label0
        # print("THIS LABEL_3", this_label - init_class[this_task], flush=True)
    ...
    # print(data[1],flush=True)
    # sys.stdout.flush()
    return data, taskcla, class_order, idxs
