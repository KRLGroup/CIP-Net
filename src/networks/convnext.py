...
from torchvision.mo._utils import _ovewrite_named_param, handle_legacy_interface
...
def compute_conv2d_output_dim(H_in, W_in, kernel_size, stride=1, padding=0, dilation=1):
    """
    Computes the output dimensions (height, width) of a Conv2D layer.
    ...
    Parameters:
        ...
        dilation (int or tuple): dilation

    Returns:
        (H_out, W_out): tuple of output dimensions
    """
    def _pair(val):
...
def compute_conv2d_output_shape(C_in, H_in, W_in, C_out, kernel_size, stride=1, padding=0, dilation=1):
    H_out, W_out = compute_conv2d_output_dim(H_in, W_in, kernel_size, stride, padding, dilation)
        ...
        **kwargs: Any,
        ...
        for cnf in block_setting:
            ...
            for _ in range(cnf.num_layers):
                ...
                stage_block_id += 1
                self.ksize.append(7)
                self.map.extend([compute_conv2d_output_dim(),cnf.input_channels])
