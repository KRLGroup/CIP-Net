...
#HO MODIFICATO IL PATH MA ANDRA RIVISTO
...
from networks.pipnet_utils.util.data import get_dataloaders #+
from torch import Tensor

# class PIPNet_head(nn.Module):
#     def __init__(self,
#                  num_classes: int,
#                  num_prototypes: int,
...
#         self.out_features = self._classification_layer #aggiunto
...
#         # DIMENSIONI FEATURES IN FORWARD PIPNET: torch.Size([64, 512, 28, 28])
#         proto_features = self._add_on(features)
#         # DIMENSIONI PROTOFEATURES IN FORWARD PIPNET: torch.Size([64, 512, 28, 28])
#         pooled = self._pool(proto_features)
#         # DIMENSIONI pooled IN FORWARD PIPNET: torch.Size([64, 512])
#         if inference:
...
def get_network(num_classes: int, args: argparse.Namespace): 
    ...
    if 'next' in args.network:
        features_name = str(args.network).upper()
