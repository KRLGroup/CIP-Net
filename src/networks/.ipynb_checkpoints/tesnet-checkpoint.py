...
class TesNet_head(nn.Module):
    def __init__(self, prototype_shape, num_prototypes, num_classes, prototype_activation_function, focal,
        ...
        if not self.share_add_ons:
            self.add_on_layers = nn.Sequential( ######################### QUESTO SAREBBE F_A nel paper
    ...
    def forward(self, conv_features):
        #DIMENSIONI conv_features IN FORWARD TESNET [80, 256, 7, 7]
        if not self.share_add_ons:
            conv_features = self.add_on_layers(conv_features) # F_A
            print("DIMENSIONI DENTRO IF CONV_FEATURES SHARE ADD ONS:",conv_features.shape)
        project_distances, cosine_distances = self.prototype_distances(conv_features) # [80, 500, 7, 7], [80, 500, 7, 7]
        cosine_min_distances = self.global_min_pooling(cosine_distances) # [80, 500]

        project_max_distances = self.global_max_pooling(project_distances) # [80, 500]
...
#         features = self._net(xs) # DOVE NET SAREBBE LA RETE CONVOLUZIONALE
